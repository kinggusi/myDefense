...
canonical Mutation ID와 전투 메커니즘은 다음처럼 고정한다.

| Mutation ID | 역할 | 전투 핵심 효과 |
|---|---|---|
| `GIANT` | 광역형 | 기본 공격 적중 지점 주변에 감쇠 광역 피해 |
| `BERSERK` | 단일 보스형 | Boss 대상 단일 피해 배율 증가 |
| `SWIFT` | 공격속도형 | 공격속도 증가 |
| `TOXIC` | 지속 피해형 | 적중 대상에 권위적 DoT 적용 |
| `GREEDY` | 경제형 | 적중 기반 개인 인게임 Gold 획득 |
| `OBESE` | 도박형 | 결정적 확률 판정으로 약/강 피해 중 하나 적용 |
| `FROZEN` | 상태 이상형 | 적중 대상 이동속도 감소 |
| `BLANK` | 꽝형 | 전투 효과 없음, Injector 풀에서는 제외 |

수치와 확률은 `MutationSpec` Excel 시트와 canonical `mutation-spec.json`만을 원천으로 사용한다. State Authority가 판정하며 클라이언트는 피해량, 확률, Gold를 임의 결정하지 않는다.
...
| 계통 | 단계별 누적 공격력 | 단계별 비용 확정값 |
|---|---|---|
| 일반 공명 | +5%, +10%, +15%, +20%, +25% | 400, 800, 1,400, 2,200, 3,200 |
| 신화 공명 | +8%, +16%, +24%, +32%, +40% | 800, 1,600, 2,800, 4,400, 6,500 |

- 위 표를 P1 구현 확정값으로 사용한다.
- 공격속도는 단계당 누적 1%씩, 5단계에서 총 5%를 추가한다.
- 사거리는 인게임 공통 강화로 올리지 않는다.
- 매치마다 0단계에서 시작하며 NORMAL 트랙은 Normal~Legendary, MYTHIC 트랙은 Mythic에만 적용한다.
...
| 왹져 DNA | 부족한 Alien 조각 대체 |
| 성장 세포 | Alien 고레벨 강화·돌파 |
| 변이 촉매 | Mutation 고레벨 강화·돌파 |

### 12.2 명칭 원칙 — 권장

- 기존 코드·JSON 필드 `universalPiece`의 표시명은 `왹져 DNA`를 사용한다.
- 코드 `growthCell`의 표시명은 `성장 세포`를 사용한다.
- 코드 `mutationCatalyst`의 표시명은 `변이 촉매`를 사용한다.
- 직렬화 하위 호환을 위해 `universalPiece` 필드명은 바꾸지 않고 Unity 표시 문자열만 `왹져 DNA`로 통일한다.
- 서버·DTO·Unity에서 `inGameGold`와 `accountGold`를 명시적으로 구분한다.

### 12.3 영구 Alien 강화 — 현행 방향

- 특정 Alien 조각과 accountGold를 사용한다.
- 부족한 전용 조각은 왹져 DNA로 일부 대체할 수 있다.
...
- 슬롯은 총 3개다.
- 1번 슬롯은 기본 제공한다.
- 2번 슬롯은 계정 레벨 30 또는 gem 5,000으로 영구 해금한다.
- 3번 슬롯은 gem 10,000으로 영구 해금한다.
- 2번 슬롯을 gem으로 먼저 해금한 뒤 레벨 30을 달성해도 환급하지 않는다.
- 시작·수령 요청은 멱등 처리해야 한다.
- 동일 Mythic 두 마리 조합 불가
- 모든 무순서 부모 조합 190개는 고정된 일반 Mythic 후보 5종을 가진다.
- 각 조합의 일반 후보 5종은 각각 19.2%, Breeding 전용 Mythic 19·20은 각각 2%다.
- `A+B`와 `B+A`는 같은 조합으로 처리한다.
- 10분 단축마다 Diamond 100개를 사용하며, 남은 시간은 올림 단위로 계산한다.
- 조합표와 확률은 공개하며 canonical Excel과 공개용 이미지에서 동일하게 관리한다.

### 13.2 후속 정책

- Breeding 포인트 천장은 최초 출시 범위에서 사용하지 않는다.
- 전용 획득 연출과 조합표 인게임 열람 UI는 후속 시각 작업에서 강화한다.
...
공통 80 Wave 템플릿에 행성별 배율을 적용한다. `9 × 80` 행을 복제하지 않으며 Wave 구성과 행성 난이도를 분리한다.
...
- Boss HP는 해당 행성·Wave 일반 HP 배율에 추가로 3배를 적용한다.
- Boss는 Wave 10·20·30·40·50·60·70·80에 `BOSS_SHARED`로 정확히 1마리 생성한다.
- 일반·Elite는 `EACH_FIELD`로 생성한다.
- 태양은 별도 잠금이 아니라 최고 배율과 패턴으로 사실상 종결 난이도를 만든다.

### 14.3 기존 권장 전투력·보상 참고표
...
- 각 콘텐츠는 하루 3회 무료 입장하며 횟수는 콘텐츠별로 독립 관리한다.
- 입장 횟수는 한국 시간(KST) 자정에 초기화한다.
- 기본 보상은 확정 지급한다.
- 최초 클리어 시 해당 Stage 기본 보상과 같은 양을 1회 추가 지급한다.
- 클리어한 Stage만 소탕할 수 있다.
- Stage 1은 기본 해금이며 이전 Stage를 클리어해야 다음 Stage가 열린다.
- 입장 시 횟수를 차감하고 일반 실패·사용자 이탈은 소모한다.
- 매칭 실패·Session 실패·`SERVER_ABORTED`만 trusted 서버 경로에서 멱등 반환한다.
- 입장·결과·반환·소탕 요청은 요청 ID 장부로 중복 차감·중복 보상을 막는다.
...
- 표의 값은 재클리어·소탕 기본 보상이다.
- 최초 클리어 총 지급량은 표의 2배다.
...
- 영구 보상 지급 전 서버가 신뢰된 matchmaking/session authority에서 관측한 2인 roster와 Settlement 참가자·맵·Balance 식별자를 대조한다. 공개 Attack Snapshot 조회는 roster 등록 side effect를 가져서는 안 된다.
- `highestClearedWave`까지의 canonical WaveSpawn을 기준으로 Monster별 총 Kill과 Boss Kill을 재계산하며, 요약이 일치하지 않으면 영구 보상을 지급하지 않는다.
- 현재 roster 저장소는 4시간 TTL의 서버 프로세스 메모리 기반이므로 만료 또는 전투 중 서버 재시작 시 Settlement를 안전하게 거부한다. 운영 Match 서비스 도입 시 인증된 영속 Session roster로 교체한다.
...
| 일일 콘텐츠 | 서버 구현 | Stage·보상·KST 입장 장부 완료, 실제 Battle 연결 필요 |
...
15. 상점 성장 상품의 구매 제한
### 21.6 80 Wave 행성 보상 정책 — 확정

- 일반 협동 매치는 행성별 80 Wave이며, 보상 기준은 현재 진행 중인 Wave가 아니라 완전히 클리어한 마지막 Wave다.
- `highestClearedWave`는 마지막으로 완전히 클리어한 Wave, `failedAtWave`는 실패가 발생한 Wave다.
- Wave 70 진행 중 실패하면 69 보상까지만 지급한다. Wave 70을 클리어한 뒤 Wave 71에서 실패하면 70 보상을 지급한다.
- 행성별 10·20·30·40·50·60·70·80 체크포인트 보상은 각 행성에서 최초 1회만 지급한다.
- 체크포인트 보상은 Gold와 기존 `universalPiece`(대체 코인)로 구성한다.
- 최초 80 Wave 클리어 젬은 해왕성 3,000부터 태양 11,000까지 행성 순서에 따라 1,000씩 증가한다.
- 같은 행성을 다시 클리어하면 최초 체크포인트·최초 클리어 젬은 지급하지 않고 반복 승리 Gold만 지급한다.
- 관전자와 탈락자는 매치 종료까지 남아 있으면 보상 대상이다. 명시적 이탈 또는 120초 이상 미복귀한 플레이어는 영구 보상 대상에서 제외한다.
- 태양은 콘텐츠 잠금이 아니라 극고난도 최종 행성으로 운영한다. 캠페인 전체 완료 보상은 후속 정책으로 보류한다.

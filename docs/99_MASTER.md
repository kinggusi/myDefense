...
- 리롤이 무료인지 inGameGold를 소비하는지
- 이전 리롤에서 본 Mythic을 전체 후보 풀이 소진되기 전에 다시 보여줄지
- 해금 Mythic이 3종 미만인 계정의 처리
...
권장안은 꽝 Mythic 2마리를 소모하고, 해금 Mythic 후보 3종 중 하나를 선택해 꽝을 제외한 랜덤 Mutation 상태로 획득하는 것이다. 다음 항목은 승인 전이다.

- 두 Mythic 소모 여부
- 결과 Mythic 선택 또는 완전 랜덤 여부
- Breeding 전용 해금 Mythic 포함 여부
- 결과 생성 위치
- 추가 리롤 여부
...
목표 매치 시간 권장안은 30~35분이다. 최종 Spawn 수는 실제 전투 시뮬레이션과 2클라이언트 테스트 후 확정한다.
...
- 동일 Mythic 두 마리 조합 허용 여부
- 전용 Mythic 2종의 정확한 확률과 천장
- Breeding 포인트 천장 도입 여부
- 남은 시간별 gem 단축 비용
- 동시에 다른 슬롯에서 같은 부모를 재사용할 수 있는지
...
```text
수성 → 금성 → 지구 → 화성 → 목성 → 토성 → 천왕성 → 해왕성 → 태양
```

- 태양은 종결 콘텐츠 맵이다.
- 단순 HP 상승뿐 아니라 행성별 전투 기믹을 추가한다.
...
- UI에서 재접속 유예 시간을 표시할지
- 연결 종료 중 수동 조작을 대행할 AI를 둘지
- 오랜 연결 종료 후에도 매치 종료 전 복귀를 허용할지
- 명시적 나가기 후 재입장을 완전히 금지할지
...
- 매치 종료 시점까지 미복귀한 플레이어의 영구 보상 지급 여부는 현재 정책상 미지급이다.

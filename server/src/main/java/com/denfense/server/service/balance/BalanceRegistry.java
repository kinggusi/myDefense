...
public class BalanceRegistry {
    ...
    public synchronized void init(GameRewardBalance rewardBalance, int maxAlienLevel, Map<Integer, AlienUpgradeCostBalance> costMap) {
        if (this.initialized) {
            throw new IllegalStateException("BalanceRegistry는 이미 초기화되었습니다.");
        }
        if (rewardBalance == null || costMap == null) {

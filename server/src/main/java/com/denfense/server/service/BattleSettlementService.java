...
public class BattleSettlementService {
 ...
 private void validate(BattleSettlementDtos.Request r){
  if(r==null||blank(r.requestId())||blank(r.battleSessionId())||blank(r.balanceVersion())||blank(r.contentHash())||blank(r.summaryHash())||!validResult(r.result())||r.players()==null||r.players().size()!=2||r.monsterKills()==null||r.startedAt()==null||r.finishedAt()==null||r.startedAt().isAfter(r.finishedAt())||r.finalWave()<0||r.finalWave()>balances.getBattleRewardBalance().maxWave()||("VICTORY".equals(r.result())&&r.finalWave()!=balances.getBattleRewardBalance().maxWave()))throw new BusinessException(ErrorCode.BATTLE_SUMMARY_INVALID);
  ...
  if(r.mapId()!=null&&(blank(r.mapId())||balances.getBattleRewardBalance().mapFirstClears().stream().noneMatch(m->m.mapId().equals(r.mapId()))))throw new BusinessException(ErrorCode.BATTLE_SUMMARY_INVALID);

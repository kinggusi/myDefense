...
public class BattleSettlementService {
 ...
 public BattleSettlementDtos.Response settle(BattleSettlementDtos.Request r){
  ...
  BattleSettlement s; try { s=writer.create(r); } catch (DataIntegrityViolationException ex) { var winner=lookup.byRequest(r.requestId()); if(winner!=null){if(!winner.getBattleSessionId().equals(r.battleSessionId())||!winner.getSummaryHash().equals(r.summaryHash()))throw new BusinessException(ErrorCode.BATTLE_REQUEST_CONFLICT);return response(winner,true);} winner=lookup.bySession(r.battleSessionId()); if(winner!=null){if(!winner.getSummaryHash().equals(r.summaryHash()))throw new BusinessException(ErrorCode.BATTLE_SETTLEMENT_CONFLICT);return response(winner,true);} throw new BusinessException(ErrorCode.BATTLE_SUMMARY_INVALID); }

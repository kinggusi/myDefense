...
public class ShopServiceImpl implements ShopService {
    ...
    /**
     * 왹져 뽑기 (가챠)
     * - 레벨업을 바로 시키지 않고 '조각(Pieces)'을 지급.
     * - 없는 왹져를 처음 뽑으면: 1개는 '몸통(1레벨)'이 되고, 나머지 중복분만 조각이 됨.
     * - 이미 있는 왹져를 뽑으면: 뽑은 개수만큼 전부 조각이 됨.
     */
    @Transactional
    public List<UserAlienResponseDto> gachaAlien(String username, int count) {
        ...
        for (Integer alienId : drawResult.keySet()) {
            ...
            if (myAlien == null) {
                ...
                UserAlien newAlien = new UserAlien(user, spec);

                // 로직: 1개는 몸통(레벨1), 나머지는 조각
                // 예: 3개 뽑음 -> 1개 몸통, 2개 조각
                newAlien.setPieces(obtainedCount - 1);

                userAlienRepository.save(newAlien);

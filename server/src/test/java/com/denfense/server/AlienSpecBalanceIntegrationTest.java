...
public class AlienSpecBalanceIntegrationTest {
    ...
    void verifyDbAndExcelMatch() {
        // 1. DB 전체 조회 (DataInit이 서버 기동 시 48건 적재함)
        List<AlienSpec> dbSpecs = alienSpecRepository.findAll();
        
        // 2. Registry 전체 조회 (alien-spec.json에서 48건 적재됨)
        List<AlienSpecBalance> excelSpecs = balanceRegistry.getAllAlienSpecs();

        // 3. 개수 검증
        assertThat(dbSpecs).hasSize(48);
        assertThat(excelSpecs).hasSize(48);
        assertThat(excelSpecs).extracting(AlienSpecBalance::alienId)
                .containsExactlyElementsOf(java.util.stream.LongStream.rangeClosed(1, 48).boxed().toList());
        assertThat(excelSpecs).filteredOn(spec -> "MYTHIC".equals(spec.grade())).hasSize(20);
        assertThat(excelSpecs).filteredOn(spec -> spec.alienId() >= 29 && spec.alienId() <= 32)
                .allMatch(spec -> !spec.isLocked());
        assertThat(excelSpecs).filteredOn(spec -> spec.alienId() >= 33 && spec.alienId() <= 48)
                .allMatch(AlienSpecBalance::isLocked);
